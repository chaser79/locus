    ...
    if (config.distanceFilter != null && config.distanceFilter! < 5) {
      warnings.add(
        const ConfigValidationWarning(
          ...
          suggestion:
              'Consider using at least 10 meters for better battery life',
        ),
      );
    }
      ...
      if (config.activityRecognitionInterval! < 0) {
        ...
        warnings.add(
          const ConfigValidationWarning(
            field: 'activityRecognitionInterval',
            message:
                'Very frequent activity updates will increase battery usage',
      ...
      if (config.maxBatchSize! < 1) {
        errors.add(
          const ConfigValidationError(
            field: 'maxBatchSize',
            message:
                'maxBatchSize must be at least 1 when batchSync is enabled',
