...
void main() {
  ...
  group('HeartbeatEmitter', () {
    ...
    test(
      'tickNow refreshes the registry quarantine gauge from the backlog',
      () async {
        ...
        final emitter = HeartbeatEmitter(
          backlogReader: () async =>
              const LocationSyncBacklog(quarantinedLocationCount: 4),
        );
        await emitter.tickNow();

        final snap = await registry.metrics.snapshot();
        expect(
          snap.pointsQuarantinedNow,
          4,
    ...
    test(
      'tickNow leaves the quarantine gauge alone when backlog read fails',
      () async {
    ...
    test(
      'emits last_success_age_ms = null when no success has been recorded',
      () async {
        final emitter = HeartbeatEmitter(
          backlogReader: () async => const LocationSyncBacklog(),
        );
