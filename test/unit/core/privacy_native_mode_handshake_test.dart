...
void main() {
  ...
  test(
    'cold construction preserves native guard until an explicit mutation',
    () async {
  ...
  test(
    'native guard failure prevents an enabled zone from becoming active',
    () async {
      final locus = MethodChannelLocus();
      await pumpEventQueue();
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
  ...
  test(
    'explicitly removing an already-empty set releases a retained guard',
    () async {
      final locus = MethodChannelLocus();
      await pumpEventQueue();
  ...
  test(
    'invalid zone fails validation without changing native privacy state',
    () async {
      final locus = MethodChannelLocus();
      await pumpEventQueue();

      await expectLater(
        locus.addPrivacyZone(
          PrivacyZone.create(
            identifier: '',
            latitude: 37.7749,
            longitude: -122.4194,
